  ...
  "AddressLookupConnector" should {
    ...
    "format the getAddressUrl correctly for" when {
      "calling getCustomerDetailsUrl" in {
        ...
        testUrl shouldBe s"${frontendAppConfig.addressLookupUrl}/api/confirmed?id=$vrn"
      }
    }
    ...
    def initaliseJourneyResult: Future[HttpPostResult[AddressLookupOnRampModel]] =
      TestAddressLookupConnector.initialiseJourney(AddressLookupJsonBuilder(continueUrl))
    ...
    "for initialiseJourney method" when {
      ...
      "when given a successful response" should {
        ...
        "return a Right with an AddressLookupOnRampModel" in {
          ...
          setupMockHttpPost(s"${frontendAppConfig.addressLookupUrl}/api/init")(successfulResponse)
          await(initaliseJourneyResult) shouldBe Right(AddressLookupOnRampModel(continueUrl))
        }
      }
      ...
      "given a non successful response should" should {
        ...
        "return an Left with an ErrorModel" in {
          ...
          setupMockHttpPost(s"${frontendAppConfig.addressLookupUrl}/api/init")(failedResponse)
          await(initaliseJourneyResult) shouldBe Left(ErrorModel(Status.INTERNAL_SERVER_ERROR,"Downstream error returned from Address Lookup"))
        }
      }
    }
  }
