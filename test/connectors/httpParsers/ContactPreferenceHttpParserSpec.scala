...
import connectors.httpParsers.ContactPreferencesHttpParser.ContactPreferencesReads
  ...
  "The ContactPreferenceHttpParser" when {
    ...
    "the http response status is OK" when {
      ...
      "preference is Digital" should {
        ...
        "return Digital" in {
          ContactPreferencesReads.read("", "", HttpResponse(Status.OK, Some(response))) shouldBe Right(ContactPreference("DIGITAL"))
        }
      }
      ...
      "preference is Paper" should {
        ...
        "return Paper" in {
          ContactPreferencesReads.read("", "", HttpResponse(Status.OK, Some(response))) shouldBe Right(ContactPreference("PAPER"))
        }
      }
      ...
      "preference is invalid" should {
        ...
        "return an ErrorModel" in {
          ContactPreferencesReads.read("", "", HttpResponse(Status.OK, Some(response))) shouldBe
    ...
    "the http response status is unexpected" should {
      ...
      "return an ErrorModel" in {
        ContactPreferencesReads.read("", "", HttpResponse(Status.NOT_FOUND, responseString = Some("Response body"))) shouldBe
