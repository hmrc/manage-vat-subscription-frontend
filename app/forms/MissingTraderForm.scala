...
object MissingTraderForm {
  ...
  val missingTraderError: String = "missingTrader.error"
  ...
  val formatter: Formatter[YesNo] = new Formatter[YesNo] {
    override def bind(key: String, data: Map[String, String]): Either[Seq[FormError], YesNo] = {
      data.get(key) match {
        ...
        case _ => Left(Seq(FormError(key, missingTraderError)))
      }
    }
  ...
  val missingTraderForm: Form[YesNo] = Form(
    single(
      yesNo -> of(formatter)
    )
  )
