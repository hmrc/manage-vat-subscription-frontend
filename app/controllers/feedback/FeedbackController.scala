...
import config.{FrontendAppConfig, VatcHeaderCarrierForPartialsConverter}
...
class FeedbackController @Inject()(implicit val config: FrontendAppConfig,
                                   ...
                                   val vatcHeaderCarrierForPartialsConverter: VatcHeaderCarrierForPartialsConverter
  ...
  private def partialsReadyHeaderCarrier(implicit request: Request[_]): HeaderCarrier = {
    val hc = vatcHeaderCarrierForPartialsConverter.headerCarrierEncryptingSessionCookieFromRequest(request)
    vatcHeaderCarrierForPartialsConverter.headerCarrierForPartialsToHeaderCarrier(hc)
  }
